...
import com.mawa.assistant.ui.main.MainActivity
import java.util.Locale

/**
 * AppLockActivity — MAWA Complete Lock Screen
 */
class AppLockActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
        ...
        if (status == TextToSpeech.SUCCESS) {
            ...
            handler.postDelayed({ 
                if (!isGeminiConnected) {
                    val mode = getSharedPreferences("mawa_prefs", MODE_PRIVATE).getString("personality_mode", "gf") ?: "gf"
                    val greeting = if (mode == "gf") "Agey unlock koro jaan, tarpor use korte dibo." else "Please unlock to continue."
                    speak(greeting, true) 
                }
            }, 3000)
        }
    ...
    private fun setupFinger() {
        ...
        if (canAuth == BiometricManager.BIOMETRIC_SUCCESS || SecurityManager.isBiometricEnabled(this)) {
            ...
            Log.d("MAWA_LOCK", "Biometric not available: $canAuth")
        }
    }
    ...
    private fun startVoiceListen() {
        ...
        speechRecognizer?.setRecognitionListener(object : RecognitionListener {
            ...
            override fun onError(e: Int) {
                ...
                speak("Bujhte parini, abar try koro", true)
            }
    ...
    private fun onSuccess() {
        ...
        val msg = when (mode) {
            "gf"           -> "Lock khule geche! Eshe gecho $name! Welcome back!"
            "professional" -> "Lock khule geche. Welcome back $name."
            else           -> "Lock khule geche! Unlock successful."
        }
        speak(msg, true)
        handler.postDelayed({ 
            finishAndRemoveTask()
        ...
        lockoutRunnable = object : Runnable {
            override fun run() {
                if (rem <= 0) {
                    ...
                    speak("Ebar try korte paro", true)
                    return
                }
    ...
    private fun speakWrong(attempts: Int, lockedOut: Boolean, type: String) {
        val mode = getSharedPreferences("mawa_prefs", MODE_PRIVATE)
            .getString("personality_mode", "gf") ?: "gf"
        val rem = 3 - attempts
        val msg = when {
            lockedOut -> when (mode) {
                "gf"  -> "Yaar! Tin bar bhul? Thamo, 30 second er jonno lock kore diyechi!"
                "professional" -> "Too many failed attempts. Locked for 30 seconds."
                else  -> "Onek bhul attempts. 30 second opekha koro."
            }
            type == "voice" -> when (mode) {
                "gf"  -> "Eta passphrase chilo na jaan! Thik kore bole try koro."
                "professional" -> "Voice passphrase mismatch. Please try again."
                else  -> "Bhul passphrase. Abar try koro."
            }
            attempts == 1 -> when (mode) {
                "gf"  -> "Arre bhul hocche! Dhyan diye dao, $rem bar sujog ache."
                "professional" -> "Incorrect. $rem attempts remaining."
                else  -> "Bhul! $rem bar aro try korte parbe."
            }
            attempts == 2 -> when (mode) {
                "gf"  -> "Abar bhul?! Ar ekbar bhul hole lock hoye jabe! Bhebe chinte!"
                "professional" -> "Second failure. One attempt remaining before lockout."
                else  -> "Abar bhul! Ekta sujog baki ache!"
            }
            else -> "Bhul! Abar try koro."
        }
        speak(msg, true)
    }
    ...
    private fun initGemini() {
        ...
        val systemPrompt = when(personality) {
            "gf" -> "You are MAWA, the user's caring and emotional girlfriend. Keep security responses short, sweet, and in Banglish. Use words like 'jaan', 'babu' occasionally but keep it professional for security."
            "professional" -> "You are MAWA, a professional security assistant. Keep responses very short, formal and in English."
            else -> "You are MAWA, a friendly assistant. Keep responses short and balanced."
        }
        ...
        geminiClient = GeminiLiveClient(apiKey, systemPrompt, object : GeminiLiveClient.LiveListener {
            ...
            override fun onConnected() { 
                ...
                Log.d("MAWA_LOCK", "Gemini WebSocket Connected ✅")
                runOnUiThread {
                    val mode = getSharedPreferences("mawa_prefs", MODE_PRIVATE).getString("personality_mode", "gf") ?: "gf"
                    val greeting = if (mode == "gf") "Agey unlock koro jaan, tarpor use korte dibo." else "Please unlock to continue."
                    speak(greeting, true)
            ...
            override fun onError(msg: String) { 
                ...
                Log.e("MAWA_LOCK", "Gemini WebSocket Error: $msg")
            }
        })
        geminiClient?.start()
    }
