...
/**
 * SecuritySettingsActivity — MAWA Security Configuration
 */
class SecuritySettingsActivity : AppCompatActivity(), android.speech.tts.TextToSpeech.OnInitListener {
    ...
    private fun setupListeners() {
        appLockSwitch.setOnCheckedChangeListener { _, checked ->
            if (checked) {
                if (!SecurityManager.hasPin(this) && !PatternManager.isPatternSet(this)) {
                    ...
                    toast("Agey PIN ba Pattern set koro")
                    speak("Agey PIN ba pattern set koro tab lock enable hobe", true)
                ...
                loadCurrentStatus()
                speak("App lock on hoye geche!", true)
            } else {
                ...
                speak("App lock bandho kore diyechi", true)
            }
        }
        ...
        fingerprintSwitch.setOnCheckedChangeListener { _, checked ->
            ...
            speak(if (checked) "Fingerprint unlock enable hoye geche" else "Fingerprint unlock bandho kore diyechi", true)
        }

        deviceLockSwitch.setOnCheckedChangeListener { _, checked ->
            SecurityManager.setDeviceLockEnabled(this, checked)
            speak(if (checked) "System screen lock enable hoye geche" else "System lock bandho kore diyechi", true)
        }
        ...
        privateModeSwitch.setOnCheckedChangeListener { _, checked ->
            if (checked) {
                ...
                speak("Private mode on. Chat history lukiye jabe.", true)
            } else {
                ...
                speak("Private mode bandho.", true)
            }
        }
    }
    ...
    private fun initGemini() {
        ...
        geminiClient = GeminiLiveClient(apiKey, "You are MAWA's security voice. Keep responses very short and professional.", object : GeminiLiveClient.LiveListener {
